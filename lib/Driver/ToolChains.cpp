...
static bool findBiarchMultilibs(const Driver &D, const llvm::Triple &TargetTriple,
                                StringRef Path, const ArgList &Args,
                                bool NeedsBiarchSuffix,
                                DetectedMultilibs &Result);

static bool findARMMultilibs(const Driver &D, const llvm::Triple &TargetTriple, StringRef Path,
                             const llvm::opt::ArgList &Args,
                             bool NeedsBiarchSuffix,
                             DetectedMultilibs &Result) {
  if (TargetTriple.getEnvironment() != llvm::Triple::Android)
    return findBiarchMultilibs(D, TargetTriple, Path, Args, NeedsBiarchSuffix,
                               Result);
  ...
  for (unsigned i = 0; i < NumLibSuffixes; ++i) {
    ...
    for (vfs::directory_iterator
      ...
      if (isMipsArch(TargetArch)) {
        ...
        if (!findARMMultilibs(D, TargetTriple, LI->getName(), Args,
